...
2. 모든 수학 용어는 반드시 정식 한국어 명칭으로 기술하십시오. 다음은 필수 매핑 번역 예시입니다:
   ...
   - expression -> 식
   - graph -> 그래프
3. 모든 수학 수식(분수, 대분수, 제곱, 루트 등)은 반드시 $...$ 기호로 감싸서 LaTeX 형식으로 작성하십시오.
   - 분수는 반드시 LaTeX의 \\frac{a}{b} 형태로 분자와 분모를 중괄호 {}로 각각 확실하게 감싸서 작성하십시오. \\frac25 처럼 중괄호를 생략해 쓰는 것을 엄격히 금지합니다. (예: $\\frac{7-3}{8}$, $\\frac{2}{5}$)
...
4. 수학적 정확성을 엄격히 검증하여 채점하십시오. 대분수 변환 문제 등 모든 수학적 몫과 나머지가 정확히 계산되었는지 대조하십시오. (예: \\frac{9}{4}는 몫 2, 나머지 1에 의해 정확하게 2 \\frac{1}{4}로 대분수 변환되어야 하며, 절대 2 \\frac{1}{3}이 될 수 없습니다.)
...
8. 예상 오개념(expectedMisconceptions): 학생이 왜 오답을 골랐는지 인지적 관점(예: 분모를 그냥 더했다든가, 약수를 계산하지 않았다든가 등)에서 명확하게 기술하되, 수식은 $...$로 감싸서 LaTeX 형식으로 적어주세요.
...
10. 문항별 정밀 채점 분석(questionsAnalysis): 각 문항에 대하여 채점 결과를 상세히 배열에 담으십시오.
   ...
   - correctAnswer: 모범 정답 텍스트 전체 (예: "① $1 \\frac{1}{5}$")
   - studentAnswer: 학생이 기입한 기호 텍스트 (예: "①" 또는 "②")
   - concept: 해당 문항이 테스트하는 세부 수학 개념
   - misconception: 오답 시 구체적인 인지 오류 분석 (맞은 경우 "개념을 올바르게 이해하고 적용함" 등으로 작성하고, 오답인 경우 원인이 되는 구체적인 오개념 작성. 수식이 포함되면 반드시 $...$로 감싸서 LaTeX 작성)
