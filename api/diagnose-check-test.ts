...
2. 모든 수학 용어는 반드시 정식 한국어 명칭으로 기술하십시오. 다음은 필수 매핑 번역 예시입니다:
   ...
   - expression -> 식
   - graph -> 그래프
3. 정밀 채점: 학생이 제출한 답안(예: '①', '②' 등)과 문제의 모범 답안(예: '① 1/5' 등)을 신중하게 비교하십시오. 학생이 제출한 답안 기호(①, ②, ③, ④, ⑤)가 모범 답안의 제일 앞 기호와 일치하면 정답(isCorrect = true)으로 처리하십시오.
4. 맞힌 문항 수(correctCount)를 산출해 주세요.
5. 부족한 개념(weakConcepts): 학생이 오답을 기재한 문항의 수학 개념 또는 성취도 저조가 예상되는 보완 개념들을 나열해 주세요.
6. 예상 오개념(expectedMisconceptions): 학생이 왜 오답을 골랐는지 인지적 관점(예: 분모를 그냥 더했다든가, 약수를 계산하지 않았다든가 등)에서 명확하게 기술해 주세요.
7. 보충이 필요한 이유(reasonForReinforcement): 왜 이 학생이 추가적인 단원평가와 보충 피드백을 통해 훈련해야 하는지 1~2문장으로 설득력 있게 설명해 주세요.
8. 문항별 정밀 채점 분석(questionsAnalysis): 각 문항에 대하여 채점 결과를 상세히 배열에 담으십시오.
   - number: 문항 번호 (정수)
   - isCorrect: 해당 문항 채점 결과 (true/false)
   - correctAnswer: 모범 정답 텍스트 전체 (예: "① 1/5")
   - studentAnswer: 학생이 기입한 기호 텍스트 (예: "①" 또는 "②")
   - concept: 해당 문항이 테스트하는 세부 수학 개념
   - misconception: 오답 시 구체적인 인지 오류 분석 (맞은 경우 "개념을 올바르게 이해하고 적용함" 등으로 작성하고, 오답인 경우 원인이 되는 구체적인 오개념 작성)
9. 추천 출제 조건(recommendedSettings) 설계:
