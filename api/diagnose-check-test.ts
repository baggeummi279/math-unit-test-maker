...
[중요 지시 사항 및 규칙]
1. 모든 진단 결과 항목(weakConcepts, expectedMisconceptions, reasonForReinforcement, recommendedSettings의 concepts, purpose, 그리고 questionsAnalysis 내 텍스트들 등)은 반드시 표준 한국어만을 사용하여 작성하십시오. 영어 단어, 프랑스어 등 외국어 단어, 또는 이를 음차한 외래어의 사용을 엄격히 금지합니다.
...
4. 수학적 정확성을 엄격히 검증하여 채점하십시오. 대분수로 나타내는 문제 등 모든 수학적 몫과 나머지가 정확히 계산되었는지 대조하십시오. (예: 9/4는 몫 2, 나머지 1에 의해 정확하게 2 1/4로 대분수로 나타내야 하며, 절대 2 1/3이 될 수 없습니다.)
...
8. 예상 오개념(expectedMisconceptions): 학생이 왜 오답을 골랐는지 인지적 관점(어떤 오류를 범할 수 있는지, 왜 그런 오류가 생기는지, 어떤 개념을 보완해야 하는지)에서 명확하게 기술하되, 문장은 답지/교사용 해설 문체("~할 수 있습니다", "~할 필요가 있습니다", "~을 확인해야 합니다")를 적용하십시오. 대화체는 일절 금지하며, LaTeX나 $는 일절 사용하지 말고 평문 분수/대분수로 적어주세요.
9. 보충이 필요한 이유(reasonForReinforcement): 왜 이 학생이 추가적인 단원평가와 보충 과정을 통해 훈련해야 하는지 교사용 피드백/보완 방향 관점에서 1~2문장의 단정하고 정갈한 답지체("~할 수 있습니다", "~할 필요가 있습니다", "~을 권장합니다")로 설명하십시오.
10. 문항별 정밀 채점 분석(questionsAnalysis): 각 문항에 대하여 채점 결과를 상세히 배열에 담으십시오.
   ...
   - misconception: 오답 시 구체적인 인지 오류 분석 (맞은 경우 "개념을 올바르게 이해하고 적용함" 등으로 작성하고, 오답인 경우 학생이 어떤 오류를 범했는지, 왜 그런 오류가 생겼는지, 어떤 개념을 보완해야 하는지를 답지/교사용 해설 문체["~할 수 있습니다", "~할 필요가 있습니다", "~을 확인해야 합니다"]로 상세하게 기술하십시오. LaTeX나 $는 일절 사용하지 말고 평문 분수/대분수로 작성)
